...
export default function AddContactScreen() {
  ...
  return <Screen><Header title="Add contact" eyebrow="EMERGENCY CONTACTS" /><Text style={[uiStyles.body, { color: colors.textMuted }]}>Add someone you trust to your emergency contacts.</Text><Button label="Choose from phone contacts" onPress={() => Alert.alert('Choose a contact', 'You can still type the contact’s details below.', [{ text: 'OK' }])} variant="secondary" icon="book-open" /><TextField label="Name" value={name} onChangeText={setName} placeholder="e.g. Alex Smith" /><TextField label="Phone number" value={phone} onChangeText={setPhone} placeholder="e.g. 07123 456789" keyboardType="phone-pad" /><TextField label="Relationship (optional)" value={relationship} onChangeText={setRelationship} placeholder="e.g. Support worker" /><Text style={[styles.previewLabel, { color: colors.primaryDark }]}>Preview</Text><Text style={[styles.preview, { color: colors.text, borderColor: colors.divider }]}>{name || 'Contact name'}{'\n'}{phone || 'Phone number'}{relationship ? `\n${relationship}` : ''}</Text><Button label="Save contact" onPress={save} disabled={!name.trim() || !validPhone} icon="check" /></Screen>;
}
